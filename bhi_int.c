...
#ifdef USE_GRAPE
static time_t t_last_grapefree;
static double dt_neighb_fact;
int grape_npipes = -1;
#endif
...
enum _function {_UL_HERMITE2_REGISTER_DT=0, _UL_HERMITE2_INIT_DT, _UL_HERMITE2_PREDICT_PART, _UL_HERMITE2_ADD_CLOSE,
                ...
                _UL_HERMITE2_FIND_TIMESTEPS, _UL_HERMITE2_HERM_PRED, _UL_HERMITE2_GRAPE_SEND_PARTICLE, _UL_HERMITE2_GRAPE_INIT,
                _UL_HERMITE2_FIND_NEIGHBOURS, _UL_HERMITE2_GRAPE_GETFORCES, _UL_HERMITE2_STEP_HERMITE, _UL_HERMITE2_XXXXXXXXX,
                    ...
                    double a[3], double a_[3])
{
    ...
    //for(pk = parts; pk < parts + pcount; pk++)
    {
         ...
         (r_2 < 9. * C_2G_C2 * C_2G_C2 * (p->m + pk->m) * (p->m + pk->m)))
        {
          ...
          //if(p-parts+1 == PRINT_1 || p-parts+1 == PRINT_2)
    ...
    void find_move_particles(struct particle *parts, int pcount, double *tmin_out)
    {
      ...
      for(movecount = 0; fmod(tmin, step_size[step_min]) == .0; step_min++)
          ...
          for(j = 0; j < step_count[step_min]; j++)
        {
          ...
          p->dtnext = 2.001 * p->dt;
    #ifdef USE_GRAPE
          if(p->dtnext > C_GRAPE_MAX_T_2)
            p->dtnext = 1.0005 * C_GRAPE_MAX_T_2;
    #endif
        }
          step_count[step_min] = 0;
        }
        //printf("%d:\t%e\t%e\t%e\t%e\n", step_min, step_size[step_min], tmin, p->dt, fmod(tmin, step_size[step_min]));
      assert(movecount > 0);
    ...
    void gr_jerk_com(double m2, double m1, double _x[3], double v[3], double an[3], double ha[3], double gr_a[3], double a_[3])
    {
      ...
      double n_[3] = {v[0] * _1_r - xv * _x[0] * _1_r3,
              v[1] * _1_r - xv * _x[1] * _1_r3,
              v[2] * _1_r - xv * _x[2] * _1_r3};
      ...
      double pn2_a1_ = -4.*v_2*v_2_
        + 1.5*v_2_*nv2*(3.-4.*nu) + 3.*v_2*nv*nv_*(3.-4.*nu)
        - 7.5*nv2*nv*nv_*(1-3.*nu);
      ...
      double pn2_a_  = nu * pn2_a1_
        + m * _1_r * pn2_a2_ - m * _1_r3 * xv * pn2_a2
        + 2. * m * m * _1_r2 * _1_r2 * xv * (9. + 21.75 * nu);
      ...
      double pn2_b_  = nu * pn2_b1_
        - .5 * m * _1_r * nv_ * (4. + 43.*nu) + .5 * m * _1_r3 * xv * nv * (4. + 43.*nu);
    #endif
    #ifdef PN25
      double pn25_a  = v_2 + 3. * m * _1_r;
      ...
      for(k = 0; k < 3; k++)
        {
          ...
          a_[k] += m * _1_r2 * (n_[k] * pn1_a + n[k] * pn1_a_
                    + a[k] * pn1_b + v[k] * pn1_b_
                    - 2. * _1_r2 * xv * (n[k] * pn1_a + v[k] * pn1_b)) * __1_c2;
    #endif
    #ifdef PN2
          a_[k] += m * _1_r2 * (n_[k] * pn2_a + n[k] * pn2_a_
                    + a[k] * pn2_b + v[k] * pn2_b_
                    - 2. * xv * _1_r2 * (n[k] * pn2_a + v[k] * pn2_b)
                    ) * __1_c4;
    #endif
    #ifdef PN25
          a_[k] -= 1.6*m*m*_1_r3*nu * (a[k] * pn25_a + v[k] * pn25_a_
                       - n_[k] * nv * pn25_b - n[k] * nv_ * pn25_b - n[k] * nv * pn25_b_
                       - 3. * xv * _1_r2 * (v[k] * pn25_a - n[k] * nv * pn25_b)
                       ) * __1_c5;
    #endif
        }
      _exit_function();
    ...
    void path_integral(int order, double dt, struct particle *p, double sign)
    {
      ...
      //double a_3[3] = {p->a_3[0] + p->ha_3[0] + p->gr_a_3[0], p->a_3[1] + p->ha_3[1] + p->gr_a_3[1], p->a_3[2] + p->ha_3[2] + p->gr_a_3[2]};
      /*/
      double a[3]   = {p->ha[0]   + p->gr_a[0],   p->ha[1]   + p->gr_a[1],   p->ha[2]   + p->gr_a[2]};
      double a_[3]  = {p->ha_[0]  + p->gr_a_[0],  p->ha_[1]  + p->gr_a_[1],  p->ha_[2]  + p->gr_a_[2]};
      double a_2[3] = {p->ha_2[0] + p->gr_a_2[0], p->ha_2[1] + p->gr_a_2[1], p->ha_2[2] + p->gr_a_2[2]};
      double a_3[3] = {p->ha_3[0] + p->gr_a_3[0], p->ha_3[1] + p->gr_a_3[1], p->ha_3[2] + p->gr_a_3[2]};
      */
      switch(order)
        {
        case 0:
    ...
    //     PN_TRACK_3RD                // F =     1/2 * (x1 - x0)   * (y1    + y0   )
                                       //       -1/12 * (x1 - x0)^2 * (y1'   - y0'  )
          emit_energy(-.5 * dt * p->m * (scal_prod(p->v, p->gr_a)
                         + sign * _1_6 * dt * (scal_prod(a, p->gr_a) + scal_prod(p->v, p->gr_a_))));
          break;
        case 5:
    //     PN_TRACK_5TH                // F =     1/2 * (x1 - x0)   * (y1    + y0   )
          ...
          emit_energy(-.5 * dt * p->m * (scal_prod(p->v, p->gr_a)
                         + sign * .2 * dt * (scal_prod(a, p->gr_a) + scal_prod(p->v, p->gr_a_)
                                 + sign * _1_12 * dt * (scal_prod(a_, p->gr_a)
                                            + 2. * scal_prod(a, p->gr_a_)
                                            + scal_prod(p->v, p->gr_a_2)))));
          break;
        case 7:
    //     PN_TRACK_7TH                // F =     1/2 * (x1 - x0)   * (y1    + y0   )
          ...
          emit_energy(-.5 * dt * p->m
              * (scal_prod(p->v, p->gr_a)
                 + sign * _1_14 * dt * (3 * (scal_prod(a, p->gr_a) + scal_prod(p->v, p->gr_a_))
                            + sign * _1_3 * dt * (scal_prod(a_, p->gr_a)
                                      + 2. * scal_prod(a, p->gr_a_)
                                      + scal_prod(p->v, p->gr_a_2)
                                      + sign * .05 * dt * (scal_prod(a_2, p->gr_a)
                                                   + 3. * scal_prod(a_, p->gr_a_)
                                                   + 3. * scal_prod(a, p->gr_a_2)
    ...
    double get_timestep_aarseth(double a[3], double a_[3], double a_2[3], double a_3[3],
                                double eta)
    {
    ...
    void herm_pred(struct particle *parts, int pcount, double tmin, int pred_only)
    {
      ...
      if(!pred_only)
          ...
          for(j = 0; j < movecount; j++)
        {
          ...
          // limit average number of neighbours:
          // n = (v * 2dt)^3 * f_N(r) < x,  f_N(r) = rho(r)/m = M/m * (beta + 3)/(4 * pi * R^(beta + 3)) * r^beta
          x = v_abs(p->x);

          dt = (RHO_N_3(x) * dt_neighb_fact  - _sqrt_mratio * x)
            * .5 / (v_abs(p->v) + sqrt(2. * parts[0].m / x));
    #ifndef DT_AARSETH_ALL
          if(dt < p->dtnext)
            p->dtnext = dt;
    #endif
        }
    ...
    {
      _enter_function(_UL_HERMITE2, _UL_HERMITE2_GRAPE_SEND_PARTICLE);
      double a2by18[3], a1by6[3], aby2[3];

      a2by18[0] = (p->a_2[0] + p->ha_2[0]) / 18.;
      a2by18[1] = (p->a_2[1] + p->ha_2[1]) / 18.;
      a2by18[2] = (p->a_2[2] + p->ha_2[2]) / 18.;
      a1by6[0]  = (p->a_[0]  + p->ha_[0])  / 6.;
      a1by6[1]  = (p->a_[1]  + p->ha_[1])  / 6.;
      a1by6[2]  = (p->a_[2]  + p->ha_[2])  / 6.;
      aby2[0]   = (p->a[0]   + p->ha[0])   * .5;
      aby2[1]   = (p->a[1]   + p->ha[1])   * .5;
      aby2[2]   = (p->a[2]   + p->ha[2])   * .5;
      g6_set_j_particle(C_GRAPE_CLUSID,
                index - 1, index, p->t, p->dt, p->m,
                a2by18, a1by6, aby2, p->v, p->x);
    #ifdef PRINT_2
      if(index == PRINT_1 || index == PRINT_2)
        {
          printf("### GRAPE SEND %4d:\tt  %e+%1.2e\tx0  %e\t%e\t%e\tx1p  %e\t%e\t%e\n",
             index,
             t_total(p->t),
             convert_time(p->dt, 0),
             ...
             convert_length(p->x[0], 0),
             convert_length(p->x[0], 0),
             convert_length(p->x[0]
                    + p->dt * p->v[0]
                    + p->dt * p->dt * aby2[0]
                    + p->dt * p->dt * p->dt * a1by6[0]
                    + p->dt * p->dt * p->dt * p->dt * a2by18[0] *.75
                    , 0),
             convert_length(p->x[1]
                    + p->dt * p->v[1]
                    + p->dt * p->dt * aby2[1]
                    + p->dt * p->dt * p->dt * a1by6[1]
                    + p->dt * p->dt * p->dt * p->dt * a2by18[1] *.75
                    , 0),
             convert_length(p->x[2]
                    + p->dt * p->v[2]
                    + p->dt * p->dt * aby2[2]
                    + p->dt * p->dt * p->dt * a1by6[2]
                    + p->dt * p->dt * p->dt * p->dt * a2by18[2] *.75
                    , 0));
          fflush(stdout);
        }
    #endif
