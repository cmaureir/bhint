...
//void allocCudaMem(int pcount)
//{
//    cudaMalloc((void**)&d_p,   sizeof(particle));
//    cudaMalloc((void**)&d_px,  sizeof(double) * pcount * 3);
//    cudaMalloc((void**)&d_pv,  sizeof(double) * pcount * 3);
//    cudaMalloc((void**)&d_x,   sizeof(double) * pcount * 3);
//    cudaMalloc((void**)&d_v,   sizeof(double) * pcount * 3);
//    cudaMalloc((void**)&d_a0,  sizeof(double) * pcount);
//    cudaMalloc((void**)&d_a1,  sizeof(double) * pcount);
//    cudaMalloc((void**)&d_a2,  sizeof(double) * pcount);
//    cudaMalloc((void**)&d_a_0, sizeof(double) * pcount);
//    cudaMalloc((void**)&d_a_1, sizeof(double) * pcount);
//    cudaMalloc((void**)&d_a_2, sizeof(double) * pcount);
//    cudaMalloc((void**)&d_phi,sizeof(double));
//    cudaMalloc((void**)&d_afact,    sizeof(double) * pcount);
//    cudaMalloc((void**)&d_afact_,   sizeof(double) * pcount);
//    cudaMalloc((void**)&d_io_close, sizeof(double) * pcount);
//    cudaMalloc((void**)&d_nearest,sizeof(int) * pcount);
//    cudaMalloc((void**)&d_energy, sizeof(int) * pcount);
//    cudaMalloc((void**)&d_close,  sizeof(int) * pcount);
//    cudaMalloc((void**)&d_new_close_warn,sizeof(int));
//
//    h_phi = (double*)malloc(sizeof(double) * pcount);
//    h_a0  = (double*)malloc(sizeof(double) * pcount);
//    h_a1  = (double*)malloc(sizeof(double) * pcount);
//    h_a2  = (double*)malloc(sizeof(double) * pcount);
//    h_a_0  = (double*)malloc(sizeof(double) * pcount);
//    h_a_1  = (double*)malloc(sizeof(double) * pcount);
//    h_a_2  = (double*)malloc(sizeof(double) * pcount);
//}
//
//void freeCudaMem()
//{
...
//}
//
//void setCudaData(struct particle *p,
//                 double *x,   double *v,   double *m,
//                 double *px,  double *pv,  double *phi,
//                 double *a0,  double *a1,  double *a2,
//                 double *a_0, double *a_1, double *a_2,
//                 int pcount,  int *new_close_warn)
//{
...
//    cudaMemcpy(d_x  , &x , sizeof(double) * pcount * 3  , cudaMemcpyHostToDevice);
//    cudaMemcpy(d_v  , &v , sizeof(double) * pcount * 3  , cudaMemcpyHostToDevice);
//    cudaMemcpy(d_px , &px , sizeof(double) * pcount * 3 , cudaMemcpyHostToDevice);
//    cudaMemcpy(d_pv , &pv , sizeof(double) * pcount * 3 , cudaMemcpyHostToDevice);
//    cudaMemcpy(d_m  , &m  , sizeof(double) * pcount     , cudaMemcpyHostToDevice);
//    cudaMemcpy(d_a0 , &a0 , sizeof(double) * pcount     , cudaMemcpyHostToDevice);
//    cudaMemcpy(d_a1 , &a1 , sizeof(double) * pcount     , cudaMemcpyHostToDevice);
//    cudaMemcpy(d_a2 , &a2 , sizeof(double) * pcount     , cudaMemcpyHostToDevice);
//    cudaMemcpy(d_a_0, &a_0, sizeof(double) * pcount     , cudaMemcpyHostToDevice);
//    cudaMemcpy(d_a_1, &a_1, sizeof(double) * pcount     , cudaMemcpyHostToDevice);
//    cudaMemcpy(d_a_2, &a_2, sizeof(double) * pcount     , cudaMemcpyHostToDevice);
//    cudaMemcpy(d_phi, &phi, sizeof(double) * pcount     , cudaMemcpyHostToDevice);
//}

//__device__ double get_energy_gpu(double *m, double *x, double *v, int pcount, int pos)
//{
...
//        {
//            // TO DO: v_dist
//            e -= p.m * m[i] / sqrt(v_dist(p.x,  parts[i].x,  2) + (i > 0 ? softening_par2 : .0)) * ((i >= 1) ? .5 : 1.);
//        }
//    }
//
//    return e;
...
//            if(
//                ((r_2 < r_perturb_2) &&
//                (m[i] * r1_2 > PERTURBING_FORCE_RATIO * m[0] * r_2))
//            )
//            {
...
//                }
//
//                // TO DO: race condition new_close_warn
//                *new_close_warn = 1;
//                // TO DO: get_energy_gpu!
//                //energy[i-posmin] = get_energy(parts, pcount, pos);
//            }
//
//        if((r_2<rs_2)&&(r_2<9.*C_2G_C2*C_2G_C2*(p->m + m[i])*(p->m + m[i])))
//        {
...
//        }
//
//        a0[i]  += afact[i-posmin] * x_0;
//        a_0[i] += afact[i-posmin] * (v_0 - v_x_ * x_0);
//        a1[i]  += afact[i-posmin] * x_1;
//        a_1[i] += afact[i-posmin] * (v_1 - v_x_ * x_1);
//        a2[i]  += afact[i-posmin] * x_2;
//        a_2[i] += afact[i-posmin] * (v_2 - v_x_ * x_2);
//        phi[i] -= afact[i-posmin] * r_2;
//        //
//    }
//}
//
...
//    //
//    // a0, a1, a2 will be only one array a
//    // a_0, a_1, a_2 will be only one array a_
//
//    int nthreads = 256;
//    int nblocks = ceil(n/(float)nthreads);
//
//    setCudaData(p, parts->x, parts->v, parts->m,
//                px, pv, h_phi,
//                h_a0,  h_a1,  h_a2,
//                h_a0,  h_a_1, h_a_2,
//                pcount, new_close_warn)
//
//    foo <<< nblocks, nthreads >>> (p, parts->x, parts->v, parts->m,
//                                   px, pv, h_phi,
//                                   h_a0,  h_a1,  h_a2,
//                                   h_a0,  h_a_1, h_a_2,
//                                   pcount, new_close_warn)
...
//    cudaMemcpy(h_phi    , d_phi     , sizeof(double) * n , cudaMemcpyDeviceToHost);
//    cudaMemcpy(h_a0     , d_a0      , sizeof(double) * n , cudaMemcpyDeviceToHost);
//    cudaMemcpy(h_a1     , d_a1      , sizeof(double) * n , cudaMemcpyDeviceToHost);
//    cudaMemcpy(h_a2     , d_a2      , sizeof(double) * n , cudaMemcpyDeviceToHost);
//    cudaMemcpy(h_a_0    , d_a_0     , sizeof(double) * n , cudaMemcpyDeviceToHost);
//    cudaMemcpy(h_a_1    , d_a_1     , sizeof(double) * n , cudaMemcpyDeviceToHost);
//    cudaMemcpy(h_a_2    , d_a_2     , sizeof(double) * n , cudaMemcpyDeviceToHost);
//    cudaMemcpy(afact_   ,d_afact_   , sizeof(double) * n       , cudaMemcpyDeviceToHost);
//    cudaMemcpy(io_close ,d_io_close , sizeof(double) * n       , cudaMemcpyDeviceToHost);
//    cudaMemcpy(close    ,d_nearest  , sizeof(int) * n          , cudaMemcpyDeviceToHost);
//    cudaMemcpy(nearest  ,d_energy   , sizeof(int) * n          , cudaMemcpyDeviceToHost);
//    cudaMemcpy(energy   ,d_close    , sizeof(int) * n          , cudaMemcpyDeviceToHost);
//
//
//    //TO DO: reduce phi, a0, a1, a2, a_0, a_1, a_2
//
//
//    for(i = posmin; i <= posmax; i++)
//    {
...
void iteration_cpu(struct particle parts[],int pcount, int pos, int posmin, int posmax,
                    struct particle *p, double *px, double *pv,
                    double *a0, double *a1, double *a2, double *a_0,
                    double *a_1, double *a_2, double r_vic_2, double *phi,
                    double maxforce, double r_perturb_2, double r1_2,
